...
class DAEAddNoise(object):
    def __init__(self, mask_ratio=0.75):
        self.mask_ratio = mask_ratio

    def __call__(self, tensor):
        # print('DEBUG!! image shape:', tensor.shape)
        c, h, w = tensor.shape
        mask = torch.rand(1, h//9, w//9).to(tensor.device)
        mask = mask > self.mask_ratio
        mask = mask.type(torch.FloatTensor)
        mask = F.upsample(mask.unsqueeze(0), size=(h, w), mode='nearest').to(tensor.device)
        tensor_masked = tensor * mask.squeeze(0)
        return tensor_masked  # , mask


class AugBackground(object):
    def __init__(self, mean=0.9, std=0.15):
        self.std = std
        self.mean = mean

    def __call__(self, img, mask):
        weight = torch.ones(img.shape)
        weight[1:, :, :] = 0
        weight[:1, :, :] = self.mean
        background = torch.normal(mean=weight, std=self.std)
