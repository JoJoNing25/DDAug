...
def train():
    ...
    model.to(device)
    criterion = exp.config.criterion()
    ...
    for epoch in range(exp.config.epochs):
        ...
        # Epoch Eval Function
        if misc.get_rank() == 0:
            logger.info("="*20 + "Val Epoch %d" % (epoch) + "="*20)
        model.eval()
        val_loss, val_acc, _ = evaluate(model, val_loader, scaler, exp, args)
        is_best = False
        if val_acc > best_acc:
            best_acc = val_acc
            is_best = True
        ...
        if exp.config.metric == 'binary_multi_cls':
            payload = 'Eval Val Loss: %.4f Eval Val mAP: %.4f Best Val mAP: %.4f' % \
                (val_loss, val_acc, best_acc)
        else:
            payload = 'Eval Val Loss: %.4f Eval Val Acc: %.4f Best Val Acc: %.4f' % \
                (val_loss, val_acc, best_acc)
        if misc.get_rank() == 0:
        ...
        test_loss, test_acc, _ = evaluate(model, test_loader, scaler, exp, args)
        if exp.config.metric == 'binary_multi_cls':
            payload = 'Test Loss: %.4f Test mAP: %.4f' % (test_loss, test_acc)
        else:
            payload = 'Test Loss: %.4f Test Acc: %.4f' % (test_loss, test_acc)
        if misc.get_rank() == 0:
            logger.info('\033[33m'+payload+'\033[0m')
...
def main():
    # Set Global Vars
    global train_loader, val_loader, test_loader, data
    global exp, logger
